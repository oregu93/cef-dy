...
Приоритет источников текущего состояния проекта:
...
Не обновляйте `PROJECT_STATE` после каждого запуска оптимизатора, smoke test или технического исправления.
...
Checkpoint создаётся после каждой содержательно значимой Work-задачи.
...
- Названия химических веществ, материалов и ионов в обычном тексте и заголовках
  записываются без LaTeX: `Dy3+`, `Fe3+`, `DyFeO3`, `RFeO3`.
...
ChatGPT при подключённом GitHub-репозитории также использует зафиксированную
ветку `main` как текущую каноническую базу знаний. Это позволяет сохранять один
