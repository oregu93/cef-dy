...
## 13. Контролируемая научная пересматриваемость

Замороженное правило или specification является авторитетным для работы под
его версией. Воспроизводимость этой работы не означает, что правило объявлено
неизменной научной истиной.

Основанием для пересмотра могут быть:

- новые evidence или reasoning;
- новые experimental evidence;
- технический или reproducibility defect;
- выявленная внутренняя inconsistency;
- избыточно ограничивающее assumption;
- лучше обоснованный inference.

Контролируемый пересмотр выполняется в последовательности:

```text
идентифицировать frozen rule и его version
→ представить evidence / reasoning
→ отделить scientific/technical основание от удобства
→ определить минимальную revision
→ указать затронутые и незатронутые prior results
→ пройти Project Control review
→ выпустить superseding frozen version
→ сохранить predecessor и provenance
```

Запрещены silent bypass, implicit workaround, ретроактивное переписывание
предшествующей версии и revision только из-за неудобства действующего правила.

## 14. Selective R&D method transfer

В проект переносится конкретный полезный метод, а не весь внешний framework.
Перед переносом оцениваются решаемая проблема, evidence полезности, минимальная
адаптация, сложность, обратимость и критерий успеха.
...
Престиж инструмента, проекта или сообщества сам по себе не является основанием
для adoption.
