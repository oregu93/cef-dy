...
**Следующий научный шаг.** `W02-02R-C-001`: После validation, canonical authorization commit и push выполнить только W02-02R-C-001. Перед первым вычислительным шагом при синхронизированном чистом HEAD запустить `python scripts/work_recovery.py start --job W02-02R-C-001`. C-001 ещё не выполнялся; C-002 и детекторный доступ к отложенной выборке не авторизованы. После STOP_CONDITION вернуть checkpoint и artifacts в «02 - TAIPAN Data Reduction» для научно-методологического review.
