...
**Сейчас.** W02-02R-B-001 и T-02R-04 завершены. Рецензированный результат принят с ограничениями: каталог содержит восемь BF в режиме monitor_controlled, все Tier-1. Физическое CEF-назначение не выполнялось; документация приведена к научному состоянию после B-001 без изменения R-012 / EV-007.

**Почему.** Проверены 57 окончательных исходов: 56 полных bootstrap B=2048 и одно окончательное численное исключение. Все 16 тестов PASS; каталог неизменяем. Перекрытия BF и численные отказы отдельных окон ограничивают выводы. Данные отложенной выборки не использовались на поисковом этапе; замены исключённого скана не было.

**Следующий шаг.** T-02R-05: только проектирование и научное рецензирование подтверждающего анализа по неизменяемому каталогу B-001. Исполнение не авторизовано; детекторные данные всех 18 сканов отложенной выборки (`holdout`) недоступны до отдельного разрешения.
