...
**Сейчас.** Stage 00B -- локальное развёртывание Knowledge Base v2.1 (структура директорий, Obsidian vault, Git repository и безопасная синхронизация между рабочим компьютером и ноутбуком).

**Почему.** Перед продолжением Stage 03D необходимо зафиксировать переносимую и воспроизводимую инфраструктуру проекта, чтобы дальнейшие расчёты, checkpoints и научные решения сразу сохранялись в устойчивой системе.

**Следующий шаг.** Развернуть локальную директорию CEF_Dy, проверить Knowledge Base в Obsidian, настроить внешний CEF_Dy_Data, инициализировать Git и выполнить первый проверенный commit/push. После этого перейти к design review Stage 03D.

**Следующий Work job.** `W03-03D-A-001 — реализация архитектуры Stage 03D и только smoke-тесты; производственная подгонка, профили и ансамбль запрещены до отдельной проверки.`

**Заблокировано.**
- Производственная оптимизация Stage 03D до утверждения формальной спецификации.
- Переход от smoke-тестов к производственным расчётам без проверки checkpoint.
