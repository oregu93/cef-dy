...
# Дополнение 2026-09-02 — repair stable control identifiers

При Stage 00C часть исторических `Q-###` и `RSK-###` была непреднамеренно
переиспользована с новым смыслом.

Stable IDs не должны менять семантику после публикации в canonical KB.

Выполнена миграция:

- новые Stage 00C/02R questions перенесены в `Q-010`–`Q-014`;
- восстановлена историческая семантика `Q-001`–`Q-009`;
- новые risk concepts перенесены в `RSK-008`–`RSK-010`;
- восстановлена историческая семантика `RSK-001`–`RSK-007`.

Также восстановлена исходная семантика Stage 00C task numbering после
временного coalescing управляющих строк.