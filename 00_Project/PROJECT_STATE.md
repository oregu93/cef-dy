...
**Следующий шаг.** `Project-Control-rerun-authorization-review`: Передать скорректированную реализацию C-001 и диагностическое происхождение завершённой с отказом попытки A1 в «00 - Project Control» для отдельного решения об авторизации повторного запуска. Повторный запуск C-001, C-002 и детекторный доступ к отложенной выборке не авторизованы.
...
Спецификация `W02-02R-C-001` для T-02R-05 методологически одобрена и
заморожена; открытых вопросов проектирования нет. Первая попытка A1 завершена
по причине `frozen_numerical_failure`: использованная реализация признана
несоответствующей спецификации, а её научный результат отклонён как
канонический. Детекторные поля отложенной выборки не открывались.

Скорректированная реализация прошла итоговый статический review с решением A и
готова к отдельному рассмотрению повторной авторизации. Это решение review не
авторизует A2: `rerun_authorization_status: not_authorized`, канонический
научный результат C-001 не установлен. Исполнение C-002 и детекторный доступ
ко всем 18 сканам отложенной выборки также остаются неавторизованными.

Диагностическое происхождение A1:
[W02-02R-C-001-A1](../02_Work_Checkpoints/W02-02R-C-001-A1.md).

Каноническая спецификация:
[W02-02R-C-001](../03_Protocols/STAGE02R_T02R05_C001_CONFIRMATORY_MODEL_PREPARATION_SPEC.md).
