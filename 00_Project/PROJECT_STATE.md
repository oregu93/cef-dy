...
Для v1.1 `execution_status: not_started`,
`implementation_status: not_prepared` и `execution_authorized: false`.
Ранее принятая реализация соответствует только superseded v1 и не может
использоваться как реализация v1.1 без отдельной подготовки и conformance
review. A2, C-002 и детекторный доступ ко всем 18 сканам отложенной выборки
не авторизованы.
