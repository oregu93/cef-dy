...
# Научное состояние за 60 секунд
...
- Любая BF-запись уже сопоставлена с legacy-идентификатором F002 или F004.
- Особенность около 18.25 meV уже окончательно доказана как CEF-переход Dy3+.
- Энергии 6.45 и 27.90 meV являются результатами слепого обнаружения или подтверждёнными CEF-уровнями.
...
## 3.1. Stage 02R A-001: разведочный аудит архива
...
Роль A-001 — установить состав и структурную неоднородность исходного архива,
не извлекая спектральные признаки и не выполняя физическое назначение.


## 3.2. Stage 02R A-002: канонический слой измерений
...
Роль A-002 — преобразовать все поддержанные форматы в проверенное представление
файлов, сканов и точек измерения с сохранением исходных каналов и provenance.


## 3.3. Stage 02R A-003: совместимость режимов и конфигураций
...
Роль A-003 — определить по записанным метаданным, какие сканы допускают
совместное статистическое рассмотрение. Этот этап не выполнял численную
нормировку и не доказывал полной физической идентичности конфигураций.


## 3.4. Stage 02R B-001: рецензированный слепой каталог
...
Роль B-001 — сформировать до обращения к историческим энергиям и CEF-моделям
замороженный набор алгоритмически воспроизводимых областей локального избытка,
который может служить входом для отдельно спроектированного подтверждающего
анализа. BF-ID не являются номерами физических линий.
...
Спецификация `W02-02R-C-001 v1.1` для T-02R-05 научно-методологически
принята и заморожена. Она замещает v1 для любого будущего исполнения, но не
стирает его: v1 и диагностическое происхождение A1 сохраняются как
исторические записи.
...
Активная замороженная спецификация:
[W02-02R-C-001 v1.1](../03_Protocols/STAGE02R_T02R05_C001_V1_1_CONFIRMATORY_MODEL_PREPARATION_SPEC.md).

Историческая спецификация:
[W02-02R-C-001 v1](../03_Protocols/STAGE02R_T02R05_C001_CONFIRMATORY_MODEL_PREPARATION_SPEC.md).


# 4. Текущие экспериментальные свидетельства

В проекте существуют два разных слоя идентификаторов. `F002` и `F004` —
legacy-идентификаторы особенностей прежнего Stage 02/03 analysis. `BF-001`…
`BF-008` — новый нейтральный namespace алгоритмических кластеров B-001.
B-001 намеренно не выполнял исторического сопоставления энергий, поэтому
никакая связь `F002/F004 ↔ BF-*` в текущем состоянии не установлена.
Совпадение или перекрытие энергетических областей само по себе не создаёт
такого соответствия.

## 4.1. Спектральная особенность около 18.25 meV

На прежнем Stage 02 при слепом поиске спектральных особенностей была
выделена глобальная особенность с ID:
...
не были первоначально получены при слепом поиске спектральных особенностей
на Stage 02.
...
При прежнем слепом анализе TAIPAN также была выделена глобальная
...
## 4.6. Текущие физические гипотезы

Канонические статусы хранятся в
[HYPOTHESIS_REGISTER](HYPOTHESIS_REGISTER.yaml):

- `H-001` (`working`): особенность около 18.25 meV рассматривается как
  основной кандидат на CEF-переход Dy3+; назначение не установлено;
- `H-002` (`candidate`): различение вкладов O1/O2 в M1 может изменить
  волновые функции и картину INS-интенсивностей относительно M0;
- `H-003` (`candidate`): энергии 6.45 и/или 27.90 meV остаются историческими
  кандидатами с невосстановленным первичным литературным происхождением;
- `H-004` (`disfavored`): простое назначение особенности F004 одному чистому
  локализованному Dy3+ CEF-переходу неблагоприятно, но её микроскопическая
  природа остаётся открытой.

Ни одна из этих гипотез не повышается в статусе результатом B-001. Новый
каталог `BF-*` с ними пока не сопоставлен.
...
2. B-001 сформировал замороженный каталог из восьми BF-записей: все относятся
   к режиму `monitor_controlled` и имеют Tier-1; Tier-2 и межрежимной
   повторяемости положения нет. Эти BF-ID являются алгоритмическими кластерами
   воспроизводимости, а не доказанными отдельными физическими возбуждениями.

3. В B-001 18 сканов были заранее выделены в отложенную выборку и не
   использовались для поиска признаков. Один скан для поиска (`discovery`)
   окончательно
   исключён по замороженному правилу `nuisance_field_fit_failure` без
   повторного расчёта и замещения.

4. В legacy-анализе собственных TAIPAN данных выделена воспроизводимая
   спектральная особенность
   около $18.2$–$18.3~\mathrm{meV}$ (`F002`).

5. Профильный анализ этой legacy-особенности даёт рабочее положение около
   **18.25 ± 0.12 meV**, однако полная systematic energy uncertainty ещё
   не установлена.

6. Особенность около $18.25~\mathrm{meV}$ является главным кандидатом
   проекта на CEF-переход Dy3+, но assignment ещё требует проверки
   интенсивностями и другими наблюдаемыми.

7. В legacy-анализе существует широкая спектральная особенность около
   $44.4~\mathrm{meV}$ (`F004`),
   происхождение которой остаётся открытым.

8. Энергии $6.45$ и $27.90~\mathrm{meV}$ не являются результатами слепого
   обнаружения
   текущего TAIPAN experiment и не должны использоваться как подтверждённые
   экспериментальные CEF-levels.

9. Energy-only 15D CEF inverse problem существенно недоопределена для
   имеющегося исторического набора constraints.

10. Formal-charge / uniformly scaled PCM полезна как structural baseline,
   но не является полной microscopic CEF model.

11. INS intensities несут принципиально важную информацию о wavefunctions
   и должны участвовать в следующем основном inference cycle.

12. Magnetic exchange и microscopic exchange-charge contributions являются
...
1. число различных физических возбуждений, соответствующих восьми BF-ID;
2. CEF-принадлежность любой BF-записи;
3. эпистемическая связь legacy-идентификаторов `F002/F004` с каталогом `BF-*`;
...
10. окончательная TAS resolution model и absolute energy-calibration
   systematic;
11. достаточность CEF-only описания низкотемпературных данных;
12. величина и направление magnetic Dy–Fe exchange field;
13. независимо проверенная финальная structural model Dy/O для CEF;
14. ансамбль статистически допустимых CEF-гамильтонианов;
15. независимая magnetic validation по $g$, $M(H)$,
