...
**Следующий шаг.** `Project-Control-execution-authorization-review`: Передать materialized и принятую по Decision A реализацию C-001 v1.1 в «00 - Project Control» для отдельного execution-authorization review. Исполнение C-001/A2, C-002 и детекторный доступ к отложенной выборке не авторизованы.
...
Для v1.1 точные source/config materialized; статический review завершён с
`implementation_review_decision: A` и `implementation_status: accepted`.
При этом `execution_status: not_started` и `execution_authorized: false`:
принятие реализации не является авторизацией исполнения. A2, C-002 и
