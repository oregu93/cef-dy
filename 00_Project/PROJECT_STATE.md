...
заморожена; открытых вопросов проектирования нет. Project Control одобрил
исполнение C-001, но авторизация становится операционной только после
validation, commit и push этого capture в canonical main. C-001 ещё не
выполнялся. Перед первым вычислительным шагом обязателен
`python scripts/work_recovery.py start --job W02-02R-C-001`.

Авторизация относится только к C-001. Исполнение C-002 и детекторный доступ
ко всем 18 сканам отложенной выборки остаются неавторизованными. После
C-001 STOP_CONDITION checkpoint и artifacts возвращаются в
«02 - TAIPAN Data Reduction» для научно-методологического review.
